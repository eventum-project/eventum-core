...
from core.models.time_mode import TimeMode
from core.plugins.event.base import (BaseEventPlugin,
                                     EventPluginConfigurationError,
                                     EventPluginRuntimeError)
from core.plugins.input.base import (BaseInputPlugin,
                                     InputPluginConfigurationError,
                                     InputPluginRuntimeError)
from core.plugins.output.base import (BaseOutputPlugin,
                                      OutputPluginConfigurationError,
                                      OutputPluginRuntimeError)

core.logging_config.apply()
...
def start_input_subprocess(
    ...
    try:
        plugin_module = importlib.import_module(
            f'eventum_core.plugins.input.{plugin_name}'
        )
        input_plugin_class: type[BaseInputPlugin] = (
            plugin_module.load_plugin()     # type: ignore
        )
        input_plugin: BaseInputPlugin = (
            input_plugin_class.create_from_config(config=input_conf)
        )
...
def start_event_subprocess(
    ...
    try:
        plugin_module = importlib.import_module(
            f'eventum_core.plugins.event.{plugin_name}'
        )
        event_plugin_class: type[BaseEventPlugin] = (
            plugin_module.load_plugin()    # type: ignore
        )
        event_plugin: BaseEventPlugin = (
            event_plugin_class.create_from_config(
                config=config
            )
        )
    except ModuleNotFoundError as e:
...
def start_output_subprocess(
    ...
    for item in config:
        ...
        try:
            plugin_module = importlib.import_module(
                f'eventum_core.plugins.output.{plugin_name}'
            )
            output_plugin_class: type[BaseOutputPlugin] = (
                plugin_module.load_plugin()   # type: ignore
            )
            output_plugin: BaseOutputPlugin = (
                output_plugin_class.create_from_config(
                    config=output_conf
                )
            )
